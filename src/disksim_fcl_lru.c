...
void lru_insert1(struct cache_manager *c,struct lru_node *ln){  // fox
	struct lru_node *node, *dnode, *cnode;
	struct list_head *head, *dhead, *chead;
	struct list_head *pos, *dpos, *cpos;
	head = (struct list_head *)&c->cm_head; 
	...
	if ( ln->cn_dirty ) {
		dhead = (struct list_head *)&c->cm_dirty_head;
		dpos = dhead->next;
		if ( !list_empty(dhead)) {
			dnode = (struct lru_node *) list_entry( dpos, struct lru_node, cn_dirty_list );
			while ( ln->cn_prize < dnode->cn_prize ) {
				dpos = dpos->next;
				if ( dpos != dhead ) {
					dnode = (struct lru_node *)list_entry( dpos, struct lru_node, cn_dirty_list );
				} else 
		...
		list_add( &ln->cn_dirty_list, dpos->prev);
//		list_add( &ln->cn_dirty_list, &c->cm_dirty_head);
		c->cm_dirty_count ++;
		c->cm_dirty_free --;
	} else {
		chead = (struct list_head *)&c->cm_clean_head;
		cpos = chead->next;
		if ( !list_empty(chead) ) {
			cnode = (struct lru_node *) list_entry( cpos, struct lru_node, cn_clean_list );
			while ( ln->cn_prize < cnode->cn_prize ) {
				cpos = cpos->next;
				if ( cpos != chead ) {
					cnode = (struct lru_node *)list_entry( cpos, struct lru_node, cn_clean_list );
				} else 
		...
		list_add( &ln->cn_clean_list, cpos->prev);
//		list_add( &ln->cn_clean_list, &c->cm_clean_head);
		c->cm_clean_count++;
		c->cm_clean_free --;
	}
...
/*
	if ( ln->cn_dirty ) {	
		head = (struct list_head *)&c->cm_dirty_head;
		pos = head->next;
		if ( !list_empty(head)) {
			node = (struct lru_node *)list_entry( pos, struct lru_node, cn_dirty_list );
			while ( ln->cn_prize < node->cn_prize ) {
				pos = pos->next;
				if ( pos != head ) {	
					node = (struct lru_node *) list_entry(pos, struct lru_node, cn_list);	
				} else 
		...
		list_add( &ln->cn_dirty_list, pos->prev);
//		list_add( &ln->cn_dirty_list, &c->cm_dirty_head);
		c->cm_dirty_count ++;
		c->cm_dirty_free --;
	} else {
	
		head = (struct list_head *)&c->cm_clean_head;
		pos = head->next;
		if ( !list_empty(head)) {
			node = (struct lru_node *)list_entry( pos, struct lru_node, cn_clean_list );
			while ( ln->cn_prize < node->cn_prize ) {
				pos = pos->next;
				if ( pos != head ) {
					node = (struct lru_node *) list_entry( pos, struct lru_node, cn_list);
		...
		list_add( &ln->cn_clean_list, pos->prev);
//		list_add( &ln->cn_clean_list, &c->cm_clean_head);
		c->cm_clean_count++;
		c->cm_clean_free --;
	}
