...
double baseprize = 0; // base prize value
...
ioreq_event *fcl_create_child1 ( ioreq_event *parent, int devno, int blkno, int bcount,	unsigned int flags, double prize ) 
{
	ioreq_event *child = NULL;
	...
	child->cause = 0;
	if ( prize < 0) { 
		printf( " create_child1 prize = %.3lf\n ", prize );
		exit(0);
	}	
	child->prize = prize; 
	return child;
...
//	child = fcl_create_child (  parent, devno, blkno, FCL_PAGE_SIZE, flags ); 
	child = fcl_create_child1 (  parent, devno, blkno, FCL_PAGE_SIZE, flags, parent->prize ); 
...
int _fcl_replace_cache ( ioreq_event *parent, int watermark, int replace_type ) {
	...
	remove_ln = CACHE_REPLACE1(fcl_cache_mgr, watermark, replace_type); 
	...
	if (remove_ln->cn_prize) {
		baseprize = remove_ln->cn_prize; // set baseprize as victim's prize 
	}
	...
	{ 
		...
		CACHE_INSERT1 ( fcl_cache_mgr, remove_ln ); 
...
void fcl_make_normal_req (ioreq_event *parent, int blkno) {
	...
	ln->cn_prize = parent->prize; //  set ln's prize 
	CACHE_INSERT1(fcl_cache_mgr, ln); 
}

void fcl_make_normal_req1 (ioreq_event *parent, int blkno) { 
	struct lru_node *ln = NULL;
	int hit = 0;
	int goHDD = 0; 	
...
void _fcl_make_stage_req ( ioreq_event *parent, struct lru_node *ln, int list_index ) {
	...
	fcl_generate_child_request ( parent, HDD, reverse_get_blk(SSD, ln->cn_ssd_blk), READ, 0, 0); 
	fcl_generate_child_request ( parent, SSD, ln->cn_ssd_blk, WRITE, 5, READ); 
...
void _fcl_make_destage_req ( ioreq_event *parent, struct lru_node *ln, int list_index ) {
	...
	//fcl_generate_child_request ( parent, HDD, reverse_get_blk( SSD, ln->cn_ssd_blk), WRITE, list_index++, 0);
	fcl_generate_child_request ( parent, SSD, ln->cn_ssd_blk, READ, 4, 0); 
	fcl_generate_child_request ( parent, HDD, reverse_get_blk( SSD, ln->cn_ssd_blk), WRITE, 1, 0); 
...
void fcl_make_pending_list ( ioreq_event *parent, int op_type ) { 
	...
	for (i = 0; i < page_count; i++){
		...
		//child = fcl_create_child (  parent, devno, blkno, FCL_PAGE_SIZE, flags ); 
		child = fcl_create_child1 (  parent, devno, blkno, FCL_PAGE_SIZE, flags, prize ); 
		child->fcl_parent = parent;
...
*/
			fcl_make_normal_req1 ( parent, blkno ); 
			break;
...
void fcl_get_next_request ( int op_type ) {
	...
	if ( op_type == FCL_OPERATION_NORMAL ) {
		...
		req->prize = calculate_prize(req->blkno, baseprize, req->bcount, req->flags ); 
...
// printf(" blkno:%d\t length:%d\t R/W:%d\t parent->time:%d\n",parent->blkno, parent->bcount, parent->flags);
...
void fcl_request_arrive (ioreq_event *parent){
	...
	if ( ++debug_arrive % 100000 == 0 ) {
		...
		printf(" cm_free = %.2fMB, ", (double) (fcl_cache_mgr->cm_free)/256); 
		printf(" caching space = %.2fMB, ", (double) (fcl_cache_mgr->cm_size)/256); 
...
ioreq_event *fcl_create_parent (int blkno,int bcount, double time, int flags, int devno) {
	...
	parent->busno = 0;
	parent->cause = 0;
	parent->prize = calculate_prize( blkno, baseprize, bcount, flags); 
	if ( parent->prize < 0) { 
		printf( " fcl_create_parent prize = %.3lf\n ", parent->prize );
		exit(0);
	}	
	return parent;
...
void fcl_init () {
	...
	//sd_init(fcl_params->fpa_seq_detection_enable,  fcl_params->fpa_seq_unit_size ); 
}
...
void fcl_exit () {
	...
	printf("LRU list count(SSD usage): %d (%.2fMB)\n", (fcl_cache_mgr->cm_dirty_count+fcl_cache_mgr->cm_clean_count), (double)(fcl_cache_mgr->cm_dirty_count+fcl_cache_mgr->cm_clean_count)/256); 

	fprintf(outputfile, "nodeCnt = %d\n", get_nodeCnt());
	fprintf( outputfile, "WAF = %.2f\n", waf );
	fprintf(outputfile, "FCL Written = %.2fMB, Read = %.2fMB \n", (double)fcl_stat->fstat_io_write_pages/256, (double)fcl_stat->fstat_io_read_pages/256);
	fprintf(outputfile, "Caching space = %.2fMB\n", (double) (fcl_cache_mgr->cm_size)/256 );
	fprintf(outputfile, "SSD usage = %.2fMB\n", (double)(fcl_cache_mgr->cm_dirty_count+fcl_cache_mgr->cm_clean_count)/256);
  
//	lru_print_screen(fcl_cache_mgr); 
...
#undef fprintf 
	...
	// ASSERT ( fcl_stat->fstat_arrive_count == fcl_stat->fstat_complete_count );
		...
		printf( " fcl_stat->fstat_arrive_count == fcl_stat->fstat_complete_count!!!!!!!!!!\n");
